...
bool startup(Graphics* window)
{
	...
	screen = SDL_LoadBMP("bWinner.bmp");
	if (!screen)
	{
		printf("Failed to load bWinner.bmp");
		return false;
	}
	window->bWinner = SDL_CreateTextureFromSurface(window->renderer, screen);
	if (!window->bWinner)
	{
		printf("Failed to create Black Winner texture.\n");
		return false;
	}

	screen = SDL_LoadBMP("wWinner.bmp");
	if (!screen)
	{
		printf("Failed to load wWinner.bmp");
		return false;
	}
	window->wWinner = SDL_CreateTextureFromSurface(window->renderer, screen);
	if (!window->wWinner)
	{
		printf("Failed to create White Winner texture.\n");
		return false;
	}

	screen = SDL_LoadBMP("draw.bmp");
	if (!screen)
	{
		printf("Failed to load draw.bmp");
		return false;
	}
	window->draw = SDL_CreateTextureFromSurface(window->renderer, screen);
	if (!window->draw)
	{
		printf("Failed to create draw texture.\n");
		return false;
	}
...
// announce which player won via a box in the center of screen
void drawWinner(Graphics* window, int winner)
{
	...
	if (winner == 1)
	{
		SDL_RenderCopy(window->renderer, window->bWinner, NULL, &center);
	}
	else if (winner == 2)
	{
		SDL_RenderCopy(window->renderer, window->wWinner, NULL, &center);
	}
	else if (winner == 0)
	{
		SDL_RenderCopy(window->renderer, window->draw, NULL, &center);
	}
...
void shutdown(Graphics* window)
{
	...
	SDL_DestroyTexture(window->bWinner);
	SDL_DestroyTexture(window->wWinner);
	SDL_DestroyTexture(window->draw);
	SDL_DestroyTexture(window->menu.menu);
	SDL_DestroyTexture(window->menu.exitTexture);
	SDL_DestroyTexture(window->menu.loadGTexture);
	SDL_DestroyTexture(window->menu.newGTexture);
