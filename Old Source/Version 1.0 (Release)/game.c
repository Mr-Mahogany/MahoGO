...
int playGame(Game* game)
{
	...
	// check what turn the game started on to determine the player
					...
					if (xcoord < 960)
					{
						...
						for (int i = 0; i < boardsize; i++)
						{
							for (int j = 0; j < boardsize; j++)
							{
								...
								proposed.board[i][j].liberties = -2;
							}
						}
						...
						else
						{
							...
							for (int i = 0; i < boardsize; i++)
							{
								for (int j = 0; j < boardsize; j++)
								{
									...
									// checks if an opponent piece was captured, if so the atari rule may apply, so we set the piece's player to 0
...
int newGame(Game* game)
{
	...
	// set game turn count to 0
	game->turncount = 0;

	// set prisoner counts to 0
	game->prisoners[0] = 0;
	game->prisoners[1] = 0;
...
// saves the current game state for later loading
bool saveGame(Game* game)
{
	FILE* save = fopen("save.mah", "w+" );
	if (!save)
	{
	...
	fwrite(&game->turncount, sizeof(int), 1, save);
	fwrite(&game->prisoners[0], sizeof(int), 1, save);
	fwrite(&game->prisoners[1], sizeof(int), 1, save);

	for (int i = 0; i < boardsize; i++)
	{
		for (int j = 0; j < boardsize; j++)
		{
			fwrite(&game->board[i][j].player, sizeof(int), 1, save);
		}
	}
	fclose(save);
	return true;
...
// loads the previously saved file to continue the game
void loadGame(Game* game)
{
	FILE* save = fopen("save.mah", "r");
	if (!save)
	{
		newGame(game);
		return;
	...
	fread(&game->turncount, sizeof(int), 1, save);
	fread(&game->prisoners[0], sizeof(int), 1, save);
	fread(&game->prisoners[1], sizeof(int), 1, save);

	for (int i = 0; i < boardsize; i++)
	{
		for (int j = 0; j < boardsize; j++)
		{
			fread(&game->board[i][j].player, sizeof(int), 1, save);
		}
	}
